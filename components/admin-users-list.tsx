...
import { MessageCircle, Trash2, Users, ChevronDown, UserCheck } from "lucide-react"
      ...
      <div className="space-y-3">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="cleared">Zerados</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="pending" className="space-y-3">
            {filteredUsers.filter((user) => user.total > 0).length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">Nenhum usuário com dívidas pendentes</p>
              </div>
            ) : (
              filteredUsers
          ...
          <Card key={user.id} className="hover-lift">
            ...
            {/* Pagination */}
            {filteredUsers.filter((user) => user.total > 0).length > itemsPerPage && (
              <div className="flex justify-center gap-2 mt-4">
                ...
                <span className="text-sm text-muted-foreground self-center">
                  Página {currentPage} de {Math.ceil(filteredUsers.filter((user) => user.total > 0).length / itemsPerPage)}
                </span>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setCurrentPage(currentPage + 1)}
                  disabled={currentPage * itemsPerPage >= filteredUsers.filter((user) => user.total > 0).length}
                >
                  Próxima
                </Button>
              </div>
          ...
          <TabsContent value="cleared" className="space-y-3">
            {filteredUsers.filter((user) => user.total === 0).length === 0 ? (
              <div className="text-center py-8">
                <UserCheck className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                <p className="text-muted-foreground">Nenhum usuário com dívidas zeradas</p>
              </div>
            ) : (
              filteredUsers
                .filter((user) => user.total === 0)
